...
public:
    ...
    struct AccessPointers
    {
        ...
        std::map<int, std::string> msgstrings;
        std::map<int, std::string> msgstringshash;
        int engine;
         ...
         */
        const char* Request(int id) const;
         ...
         */
        int Request(const char* string) const;
    };
     ...
     */
    virtual void Multicast (MsgEntry* me, 
        const std::vector<PublishDestination>& multi, uint32_t except, float range) = 0;
     ...
     */
    void ProcessNetwork (std::chrono::milliseconds timeout);
    ...
    int GetPing(void) { return netInfos.GetAveragePingTicks(); }
    ...
    void SetMsgStrings(std::map<int, std::string> msgstrings, 
        std::map<int, std::string> msgstringshash)
    {
     ...
     */
    std::string LogMessageFilter(const char *arg);
...
protected:
        ...
        unsigned int maxsize)
    {
        ...
        if (SOCK_SELECT(std::max(mysocket, pipe_fd[0]) + 1, &set, NULL, NULL, &timeout) < 1)
        {
            timeout = prevTimeout;
            return 0;
        }
     ...
     */
    std::shared_ptr<MsgEntry> CheckCompleteMessage(uint32_t client,uint32_t id);
    ...
    std::shared_ptr<NetPacketQueueRefCount> NetworkQueue;

    /** weak referenced list of outbound queues with waiting data so disconnected clients won't receive packets*/
    GenericRefQueue<NetPacketQueueRefCount, std::shared_ptr> senders;

    /** Incoming message queue vector */
    std::vector<MsgQueue*> inqueues;

    /** Packets Awaiting Ack pool */
    std::map<std::shared_ptr<psNetPacketEntry>, PacketKey> awaitingack;
    ...
    std::chrono::milliseconds lastSendReport;
...
private:
    ...
    std::map<std::shared_ptr<psNetPacketEntry> , PacketKey> packets;

    /** for generating random values (unfortunately the msvc rand() function
     * is not good at all
     */
    std::mt19937 randommt;
    std::uniform_int_distribution<std::mt19937::result_type> randomgendist;
    ...
    std::vector<int> logmessagefilter;
...
class NetBase::Connection
{
    uint32_t sequence;
public:
    std::map<uint32_t, int> packethistoryhash;
    ...
    std::chrono::milliseconds lastRecvPacketTime;
...
class NetPacketQueueRefCount : public NetPacketQueue
{
private:
    bool pending;
